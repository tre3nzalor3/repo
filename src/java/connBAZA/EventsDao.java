...
public class EventsDao {
    ...
    public void createEvents(String tytul,String opis) {
		cm.executeUpdate("INSERT INTO events (Id,Tytul,Opis) VALUES (NULL,\""+tytul+"\","+"\""+opis+"\")");
	}
    
    public Events getEventsById(String id) {
		 resultSet = cm.executeQuery("SELECT Tytul,Opis FROM events WHERE id like \""+id+"\"");
		try {
			resultSet.next();
			return new Events(id,resultSet.getString(1),resultSet.getString(2));
    ...
    public List<Events> getEventsByUser(String userId) throws SQLException {
		...
		while(resultSet.next()) {
			event.add(new Events(resultSet.getString(1).toString(),resultSet.getString(2).toString(),resultSet.getString(3).toString()));
		}
		return event;
	}
    ...
    public List<Events> getEventsList()throws SQLException {
                resultSet = cm.executeQuery("SELECT Id,Tytul,Opis FROM events");
                List<Events> event = new ArrayList<Events>();
                while(resultSet.next()){
                        event.add(new Events(resultSet.getString(1).toString(),resultSet.getString(2).toString(),resultSet.getString(3).toString()));
                }
